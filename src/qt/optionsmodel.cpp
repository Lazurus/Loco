...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLocoAmount"))
        settings.setValue("nAnonymizeLocoAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLocoAmount = settings.value("nAnonymizeLocoAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLocoAmount"))
        SoftSetArg("-anonymizelocoamount", settings.value("nAnonymizeLocoAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLocoAmount:
            nAnonymizeLocoAmount = value.toInt();
            settings.setValue("nAnonymizeLocoAmount", nAnonymizeLocoAmount);
            emit anonymizeLocoAmountChanged(nAnonymizeLocoAmount);
            break;
        default:
            break;
        }
